...
def frame_starts_to_kernel(frame_starts_us, bin_width, n_bins, interpolate=False):
    """
    Build the frame-overlap convolution kernel from frame start times.

    This is the single source of truth for kernel construction, shared by the
    overlap simulation (Data) and the deconvolution (Reconstruct) so that the
    forward operation and its inverse use *exactly* the same operator.

    The kernel is a sum of unit deltas (one per frame), so convolving a
    single-frame signal with it yields the summed counts of all frames
    (no 1/n_frames normalization -- overlapping frames add counts, which is
    the entire statistical benefit of FOBI).

    Parameters
    ----------
    frame_starts_us : array-like
        Frame start times in microseconds (cumulative, first usually 0).
    bin_width : float
        Time bin width in microseconds.
    n_bins : int
        Length of the kernel array (same as the data window). Frame positions
        are wrapped with modulo n_bins (circular convolution), which matches
        both the steady-state physics of a continuously pulsed source and the
        FFT-based deconvolution.
    interpolate : bool, optional
        If True, distribute each delta over the two adjacent bins according to
        its fractional bin position (sub-bin precision, official FOBI style).
        If False (default), place deltas at the nearest integer bin -- this
        must match how the overlap itself was simulated.

    Returns
    -------
    np.ndarray
        Kernel of length n_bins with unit total weight per frame.
    """
    frame_starts_us = np.asarray(frame_starts_us, dtype=float)
    kernel = np.zeros(n_bins)
    positions = frame_starts_us / bin_width

    if interpolate:
        for pos in positions:
            floor = int(np.floor(pos))
            rest = pos - floor
            kernel[floor % n_bins] += 1.0 - rest
            kernel[(floor + 1) % n_bins] += rest
    else:
        for pos in positions:
            kernel[int(np.round(pos)) % n_bins] += 1.0
...
class Data:
    ...
    def __init__(self, signal_file=None, openbeam_file=None, flux=None,
        ...
        self.applied_duty_cycle = None  # Duty cycle used in poisson_sample()
        self.poisson_stage = None  # 'before_overlap' or 'after_overlap'
    ...
    def overlap(self, kernel, total_time=None, freq=None, bin_width=10, poisson_seed=None, mode='superimpose', kernel_seed=None, min_gap=None):
        ...
        **OPTIONAL SECOND POISSON**: If `poisson_seed` is provided, a second Poisson
        ...
        # CORRECT WORKFLOW: Data -> Convolute -> Overlap -> Poisson (noise is
        # generated by counting the already-overlapped beam). The legacy
        # poisson-before-overlap order is still accepted for comparison.
        ...
        if isinstance(kernel, int):
            ...
            positions = self._generate_kernel(n_frames, total_time, mode, kernel_seed, min_gap)
            print(f"Generated {mode} kernel with {n_frames} frames at positions (ms): {positions}")
            # _generate_kernel returns absolute frame positions, but the kernel
            # convention downstream is inter-frame gaps (cumsum recovers the
            # positions). Convert positions -> gaps.
            kernel = np.concatenate([positions[:1], np.diff(positions)])
            # After kernel generation, apply the physical circular overlap
            mode = 'superimpose'
        ...
        if poisson_seed is not None:
            ...
            # Independent streams for signal and openbeam (see poisson_sample)
            seq = np.random.SeedSequence(poisson_seed)
            sig_rng, op_rng = [np.random.default_rng(s) for s in seq.spawn(2)]

            temp_overlapped = self.overlapped_data
            temp_op_overlapped = self.op_overlapped_data

            self.overlapped_data = self._apply_poisson(temp_overlapped, duty_cycle=1.0, rng=sig_rng)
            self.table = self.overlapped_data

            if temp_op_overlapped is not None:
                self.op_overlapped_data = self._apply_poisson(temp_op_overlapped, duty_cycle=1.0, rng=op_rng)
                self.openbeam_table = self.op_overlapped_data
    ...
    def _create_overlap(self, df, kernel, total_time, bin_width, mode='superimpose'):
        ...
        if mode == 'superimpose':
            # FOBI-style: superimpose within the SAME time window with circular
            # wraparound. In steady state a continuously pulsed source wraps
            # slow neutrons from the previous pulse into the current frame, so
            # the physical overlap operator is a CIRCULAR convolution -- which
            # is also exactly what the FFT-based deconvolution assumes. Counts
            # are SUMMED (not averaged): n frames deliver n times the neutrons,
            # which is the statistical benefit of frame overlap.
            ...
            for frame_start_us in frame_starts_us:
                start_bin = int(np.round(frame_start_us / bin_width)) % n_bins
                new_counts += np.roll(counts_array, start_bin)
                new_err_squared += np.roll(err_array**2, start_bin)
    ...
    def poisson_sample(self, duty_cycle=None, flux=None, measurement_time=None, freq=None, seed=None):
        ...
        **CORRECT WORKFLOW**: Data → Convolute → Overlap → **Poisson** → Reconstruct

        Neutrons are counted after the frames have overlapped on the detector,
        so Poisson noise is applied to the overlapped spectrum (the most recent
        processing stage). Signal and openbeam receive independent noise streams.
        ...
        >>> data.poisson_sample(flux=1e4, measurement_time=5, freq=800)
        """
        # Use most recent stage of data: overlapped > convolved > data
        # CORRECT WORKFLOW: Data -> Convolute -> Overlap -> Poisson.
        # Neutrons are counted AFTER the frames have overlapped on the detector,
        # so the Poisson noise must be applied to the overlapped spectrum.
        ...
        if duty_cycle is None:
            ...
            # Calculate duty cycle based on whether pulse_duration is set
            if self.pulse_duration is not None:
                # For PULSED source: duty_cycle = (flux_new / flux_orig) × (time_new / time_orig) × freq_new × (pulse_duration / baseline_duration)
                # This accounts for:
                # - flux_ratio: change in neutron flux
                # - time_ratio: change in measurement duration (more time = more counts)
                # - freq_new: pulsing frequency
        ...
        # Record what was applied so downstream code (Reconstruct) can build the
        # correct comparison reference.
        self.applied_duty_cycle = duty_cycle
        self.poisson_stage = ('after_overlap' if self.overlapped_data is not None
                              else 'before_overlap')

        # Independent RNG streams for signal and openbeam: sharing one seed
        # would correlate their Poisson fluctuations, artificially cancelling
        # noise in the transmission ratio.
        seq = np.random.SeedSequence(seed)
        sig_rng, op_rng = [np.random.default_rng(s) for s in seq.spawn(2)]

        # Apply Poisson to signal
        self.poissoned_data = self._apply_poisson(source_data, duty_cycle, rng=sig_rng)
        self.table = self.poissoned_data

        # Apply Poisson to openbeam at the SAME stage as the signal
        op_source = (self.op_overlapped_data if self.op_overlapped_data is not None
                    else self.op_convolved_data if self.op_convolved_data is not None
                    else self.op_data)
        if op_source is not None:
            self.op_poissoned_data = self._apply_poisson(op_source, duty_cycle, rng=op_rng)
            self.openbeam_table = self.op_poissoned_data

        return self

    def _apply_poisson(self, df, duty_cycle, seed=None, rng=None):
        ...
        poisson_counts = rng.poisson(np.maximum(scaled_counts, 0))
